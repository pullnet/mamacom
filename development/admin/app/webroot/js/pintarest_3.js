...
// なかなか外で良いプラグインなかったので自作しちゃいましたww
		...
		$.ajax({
			...
			success:function(data){
				...
				for(var s1a=buffercount;s1a<$(array["item"]).length;s1a++)
				{
					...
					//バッファなのでオフっとく
					$(array["item"]).eq(s1a).css({"position":"absolute","opacity":0,"visibility":"hidden"});
				}
				buffercount=s1a;
...
//			$(array["item"]).eq(u2).css("top",($(array["contents"]).offset().top+columheight[maxheightcolum])+"px");
			...
			$(array["item"]).eq(u2).css("opacity",1);
	...
	function pibox_effect(item,effect,time,timed,easing,index)
	{
		...
		{
			$(item).eq(index).animate({"opacity":1},time);
		}
