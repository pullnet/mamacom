...
class EmergencyController extends AppController{
	...
	public function index($page=1){
		...
		//コンテンツ情報をset
		$result=$this->Emergencies->find("all",array(
			"limit"=>$limit,
			"page"=>$page,
		));
		$this->set("result",$result);

		$totalcount=$result=$this->Emergencies->find("count");
	...
	public function edit($id=null){
		...
		if($this->request->data){
			...
			$this->Emergencies->set($post);
			if($this->Emergencies->validates()){
			...
			if($id){
				...
				$post=$this->Emergencies->find("first",array(
					"conditions"=>array(
						"Contents.id"=>$id,
					),
				));
	...
	public function delete($id){
		...
		//idでテーブルデータ取得
		$result=$this->Emergencies->find("first",array(
			'conditions' => array(
				'Contents.id' => $id,
			)
		));
